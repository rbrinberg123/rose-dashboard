...
/** Idle/sent/error styling shared by both send buttons (matches the Live
 *  Outreach / Feedback send controls). */
...
/** POST the Week Ahead digest send. Returns an error string on failure, null on
 *  success. Surfaces the server's 429 "overlapping send" message like the other
 *  send controls. */
async function postSend(payload: Record<string, unknown>): Promise<string | null> {
  try {
    const res = await fetch("/api/week-ahead/send-email", {
      method: "POST",
...
/**
 * Week Ahead digest send controls, placed on the Upcoming Meetings page
 * (super-user only). Mirrors the Live Outreach / Feedback controls:
 *
 *  • "Send Email"      → the REAL send to the whole team (team@rosecoglobal.com,
 *    a server-owned constant). Guarded by a window.confirm() so a stray click
 *    can't blast everyone. Posts { mode: "team" }.
 ...
 *    confirmation — it only goes where you type. Posts { mode: "test", recipient }.
 *
 * Each button disables itself while its send is in flight; the server also
 * rejects overlapping sends (429) and gates on super_user (the real boundary).
 */
export function SendWeekAheadControls({ userEmail }: { userEmail?: string }) {
  ...
  async function handleTeamSend() {
    ...
    const ok = window.confirm(
      "Send the Week Ahead digest to the ENTIRE team (team@rosecoglobal.com)?",
    )
    if (!ok) return
