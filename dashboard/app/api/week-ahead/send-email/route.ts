...
import { loadWeekAheadData } from "@/lib/week-ahead/load"
import { buildWeekAheadEmailHtml } from "@/lib/week-ahead/email-html"
import { claimDailySend, releaseDailySend } from "@/lib/live-outreach-send-log"

/**
 * "Week Ahead" meetings digest email — two entry points, two auth models
...
async function sendDigestTo(recipient: string): Promise<{ subject: string; meetings: number }> {
  const { data, error } = await loadWeekAheadData()
  if (error || !data) throw new Error(`Could not load v_planning_events: ${error ?? "no data"}`)

  const html = buildWeekAheadEmailHtml(data)
  const mondayLabel = data.days[0]?.dateLabel ?? ""
  const fridayLabel = data.days[data.days.length - 1]?.dateLabel ?? ""
  const subject = `Week Ahead — Meetings ${mondayLabel} to ${fridayLabel}`

  // ONE sendMail call, ONE email to the single recipient. (An empty week still
  // sends a valid "no meetings this week" digest — fine for a test.)
  await sendMail({ recipients: [recipient], subject, html })
  return { subject, meetings: data.totalMeetings }
}
...
export async function GET(request: NextRequest) {
  ...
  // DST-safe time gate: only the fire that lands at Friday 3:45–3:59 PM Eastern
