...
async function postSend(payload: Record<string, unknown>): Promise<string | null> {
  try {
    const res = await fetch("/api/time-off/send-email", {
      method: "POST",
...
/**
 * Time Off digest send controls (super-user only; the route also enforces
 * super_user server-side):
 *
 *  • "Send Email"      → the REAL send to the four TIME_OFF_RECIPIENTS (a
 *    server-owned constant). Guarded by a confirm() dialog so a stray click can't
 *    blast everyone.
 ...
 */
export function SendTimeOffControls({ userEmail }: { userEmail?: string }) {
  ...
  async function handleTeamSend() {
    ...
    const ok = window.confirm(
      "Send the Time Off digest to the team (Simon, Robert, Blair, Scott)?",
    )
    if (!ok) return
  ...
  return (
    <div className="flex items-center gap-2">
      {/* Real send — to the four recipients, behind a confirm dialog. */}
      <button
        type="button"
        onClick={handleTeamSend}
        disabled={teamSending}
        title={teamState === "error" && teamMsg ? teamMsg : "Send to the team (Simon, Robert, Blair, Scott)"}
        className={BASE_BTN}
