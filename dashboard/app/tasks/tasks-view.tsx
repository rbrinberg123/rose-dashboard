...
/**
 * The all-CRM Tasks table. Every task, no row scoping — see the security note in
 * app/tasks/page.tsx; this component only renders what it is handed.
 *
 * Same architecture as the Meetings and Events tables, on the same shared
 ...
 * layer from lib/table-views/. What is local here is only how a TASK row is
 * painted.
 *
...
import {
  ...
  getTaskColumn,
  isPersonName,
  taskCatalogBySection,
  type TaskColumnDef,
} from "@/lib/tasks/spec"
import type { TaskQuickFilters } from "@/lib/tasks/filters"
...
const QUICK_FILTERS: QuickFilterDef[] = [
  ...
  { key: "task_type", label: "Task Type", allLabel: "All types" },
  { key: "subtype", label: "Sub-type", allLabel: "All sub-types", searchable: true },
  { key: "owner", label: "Owner", allLabel: "All owners", searchable: true },
  { key: "status", label: "Status", allLabel: "All statuses" },
]

/** Header bands — one per run of consecutive columns sharing a group. */
function bandsFor(cols: TaskColumnDef[]): GroupBand[] {
  const out: GroupBand[] = []
  cols.forEach((c, i) => {
...
function bandStarts(cols: TaskColumnDef[]): Set<number> {
...
function searchKeysFor(cols: TaskColumnDef[]): string[] {
  return [...new Set(cols.filter((c) => c.type !== "date").map((c) => c.key))]
}

function haystackFor(row: AdminTaskRow, keys: string[]): string {
  return keys
    .map((k) => {
...
function matchesKeyword(rows: AdminTaskRow[], keys: string[], query: string): AdminTaskRow[] {
...
export function TasksView({
  ...
  const buildUrl = React.useCallback(
    (config: ViewConfig, quick: TaskQuickFilters) => {
      ...
      if (quick.task_type) params.set("type", quick.task_type)
      if (quick.subtype) params.set("sub", quick.subtype)
  ...
  const applyQuick = React.useCallback(
    (next: Record<string, string | undefined>) => {
      const url = buildUrl(activeConfig, next as TaskQuickFilters)
      resetScroll()
      startSwitch(() => router.push(url))
    },
  ...
  const anyQuickFilter = !!(
    quickFilters.client ||
    quickFilters.task_type ||
    quickFilters.subtype ||
    quickFilters.owner ||
    quickFilters.status
  )
  ...
  const loadOptions = React.useCallback(async () => {
    const res = await loadTaskFilterOptions()
    return res.ok ? res.data : {}
  }, [])

  const catalogSections = React.useMemo(
    () => taskCatalogBySection() as { section: string; columns: ColumnDef[] }[],
    [],
  )
  const opsForField = React.useCallback((f: string) => sharedOpsForField(TASKS_SPEC, f), [])
   ...
   *  task's values under the new one's header. */
  ...
  async function onExport() {
    ...
    try {
      ...
      let out = sorted
      if (truncated) {
        const res = await loadTaskRowsForExport({ config: activeConfig, quick: quickFilters })
        if (!res.ok) {
      ...
      await exportToExcel(out as unknown as Record<string, unknown>[], columns, {
        sheetName: "Tasks",
        fileStem: "crm-tasks",
        // The full client name and the coarse state are the first two questions
        // anyone asks of a CRM dump — the table shows a ticker and a status pill.
            ...
            {total === 0 ? (
              <TableRow>
                <TableCell colSpan={colCount} className="py-10 text-center text-muted-foreground">
                  {rows.length === 0 ? "No tasks returned." : `No tasks match “${query}”.`}
                </TableCell>
              </TableRow>
            ) : (
              <>
                ...
                {visible.map((r) => (
                  <TableRow
                    ...
                    className="cursor-pointer"
                    tabIndex={0}
                    aria-label={`Open record for ${r.subject ?? "task"}`}
                    onClick={(e) => {
                      if (!isInteractiveTarget(e.target)) openRecord(r.task_id)
                    }}
                    onKeyDown={(e) => {
                      if (e.key === "Enter" && e.target === e.currentTarget) {
                        openRecord(r.task_id)
                      }
                    }}
                  >
                    {columns.map((col, ci) => (
...
function renderCell(
  ...
  switch (col.renderer) {
    ...
    case "ticker": {
      ...
      //
      // NO account-team circles here, unlike the Events table: a task row is
      // about the task, and its Client column is a narrow reference rather than
      // the row's subject. The circles would also collide with the Owner column
      // two cells along, which is the person who actually matters on this page.
    ...
    case "owner":
      // A real NAME gets the shared initials circle, globally disambiguated. A
      // two-letter staff code or a queue name is shown as-is — see isPersonName
      // in lib/tasks/spec.ts for why.
    ...
    case "statusPill": {
      const { bg, text: fg } = taskStatusPill(text)
      return (
        <span
