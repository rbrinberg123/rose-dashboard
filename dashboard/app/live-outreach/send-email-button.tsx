...
async function postSend(payload: Record<string, unknown>): Promise<string | null> {
  try {
    const res = await fetch("/api/live-outreach/send-email", {
      method: "POST",
...
/**
 * Live Outreach digest send controls:
 *
...
export function SendEmailControls({ userEmail }: { userEmail?: string }) {
  ...
  async function handleTeamSend() {
    ...
    const ok = window.confirm(
      "Send the Live Outreach digest to the ENTIRE team (team@rosecoglobal.com)?",
    )
    if (!ok) return
