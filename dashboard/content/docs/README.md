...
| [22 — Cutover: the ownership boundary](22-cutover-ownership-boundary.md) | The `origin` / `is_test` fence that keeps the Dynamics sync and deletion sweep off dashboard-authored rows: the origin-lock trigger, `_synced_at` for dashboard rows, and the rollout order. **Prerequisite for any dashboard write.** |
