...
| [23 — Time Off (requests & approvals)](23-time-off.md) | The super-user-only CRM → Time Off page (`/time-off-requests`): Dynamics history + dashboard requests, the Pending → Approved/Denied workflow, reviewing teams (Admin → Time Off Reviewers), the per-day Full/AM/PM model, edit/delete = super-user only (never the requester), and the Alerts + OOO Summary integration. **SQL patch still to run.** |
