...
| `v_marketing_calendar` | Events for the calendar Gantt, and Client Detail's "Marketing Events & Dates" pool. | `events`, `accounts` | Active + workflow state not "Pause". **No date cutoff** — the trailing two-month window was removed 2026-09-08 so Client Detail and the To-Do List share one pool (it also widens `/calendar` to every active event, ~168 → ~838 rows); `event_dates` is free text. | `/calendar`, `/client-detail` |
