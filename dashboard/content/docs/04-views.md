...
| View | Purpose | Source | Key rules | Consumer |
|------|---------|--------|-----------|----------|
| `v_client_portfolio` | One row per **active** client — the Portfolio page. | `meetings` (Confirmed), `events`, `contracts`, `client_notes`, `accounts` | 365d/90d trailing + `meetings_next_3m` forward counts; market-cap & region CASE buckets; **`note_status` = "last non-blank status wins"** (blank notes filtered before `DISTINCT ON`); `WHERE state_label='Active'`. Plus three appended columns (2026-09-03): **`open_slots`** = `SUM(GREATEST(of_slots − confirmed meetings, 0))` over the client's `Active` events in stage `Pre-Launch` / `Live Outreach` / `Meetings Ongoing` only; **`intro_meetings`** = distinct institutions ever met (Confirmed, all-time) and **`followup_meetings`** = total − intro. Plus **`next_meeting_date`** (2026-09-03b) = soonest Confirmed meeting whose **Eastern day is today-or-later**, NULL when nothing is booked ahead — a day comparison, deliberately unlike `meetings_next_3m`'s strictly-forward `> now()`. Plus **`meetings_ytd`** (2026-09-22) = `meetings_last_365d`'s rule over a **Jan-1-to-today** window — identical source, Confirmed filter, date field, `<= CURRENT_DATE` bound and timezone, with only the window start changed to `date_trunc('year', CURRENT_DATE)`, so **YTD ≤ L12M always**. (Not the same cut as `v_client_todo.meetings_ytd`, which splits today by `now()` on the Eastern day.) See [Portfolio columns](02-pages.md#portfolio-columns). | `/portfolio` |
...
| `v_client_detail_recent_note` | The client's newest note (may be blank). | `client_notes` | Ranked `note_date, modified_on, created_on`; `rn=1` — that latest note supplies `notes_text` and the action fields. `status_text` and `primary_risk_driver` **do carry forward** independently ("last non-blank value wins"), matching Portfolio; see `sql/patches/2026-08-27_note_status_risk_carry_forward.sql`. | `/client-detail`, `/portfolio` (note hover popover) |
...
| View | Purpose | Source | Key rules | Consumer |
|------|---------|--------|-----------|----------|
| `v_feedback_outstanding` | **Collection** tracker: concluded meetings whose feedback isn't closed. | `meetings` (Confirmed, Active), `accounts` | Incomplete = `feedback_status_label IS NULL` OR "Awaiting Additional"; only meetings whose Eastern date is **before** Eastern today. Included when the meeting has a **host OR a named feedback assignee** (`_raw->>'_bcs_feedback_value'`) — previously host-only, which dropped host-less meetings that had an owner. Responsible person = that assignee then host; the assignee is also exposed as trailing `feedback_id` / `feedback_name` columns for the UI. | `/feedback` |
...
| `v_planning_events` | Confirmed meetings of upcoming **events**. | `meetings` (Confirmed, event not null), `accounts` | Grouped by `event_id`; "upcoming" & `is_past` use **Eastern** calendar days (`(meeting_date AT TIME ZONE 'America/New_York')::date`), matching the canonical occurred rule; `meeting_day` stays UTC-read for display; exposes logistics columns (`sent`/`confirm`/`driver`/`food_order`/`logistics_notes`), `client_ticker` (from `accounts.ticker_symbol`, for the Planning V2 clickable Client column), plus `investor_text` and `hosted_in_hq` (added for the Week Ahead email digest — `hosted_in_hq` is the authoritative "in the office" flag; the /planning pages ignore both). | `/planning`, `/planning-v2`, Week Ahead email loader |
...
| `v_marketing_calendar` | Events for the calendar Gantt, and Client Detail's "Marketing Events & Dates" pool. | `events`, `accounts` | Active + workflow state not "Pause". **No date cutoff** — the trailing two-month window was removed 2026-09-08 so Client Detail and Outreach Status share one pool (it also widens `/calendar` to every active event, ~168 → ~838 rows); `event_dates` is free text. | `/calendar`, `/client-detail` |
| `v_live_outreach` | Events in the "Live Outreach" state. | `events`, `accounts`, `client_notes`, `contracts`, `meetings` | `slots_remaining = of_slots − confirmed`; `event_mode` from `event_location` text; `client_status_label` via the same carry-forward as Portfolio; `is_new_client` = earliest contract within 6 months. **Mining events are excluded** (2026-09-21): `AND COALESCE(NULLIF(e._raw ->> 'bcs_mining', '')::boolean, false) = false` — the Dynamics Yes/No toggle on the **event**, read from `_raw` because the column is newer than the mirror table. Null / absent / blank all count as false, so only events explicitly ticked **Mining = Yes** are dropped. | `/live-outreach` |
| `v_client_onboarding` | Active clients whose first feedback report has not been sent. | `accounts` (active), `tasks` | **Exit gate:** `NOT EXISTS` a `tasks` row for the client (`bcs_account_id`) with `bcs_task_subtype_label = 'Feedback Report Sent'` and `state_label = 'Completed'` — the first completed report drops the client off. Scoped to `onboarding_start_date ≥ 2026-01-01`. The 9 step flags + `filled_count` are **progress display only**, not the gate (they replaced a `filled_count < 9` gate that had gone inert). **Steps:** 6 booleans off `accounts`, 2 date fields (`onboarding_call`, `teach_in_date`) that also expose `onboarding_call_date` / `teach_in_date` for display, and `f_meeting_history` — derived from the latest **Completed** Outreach → Data Upload task via the `data_upload` CTE (same one as `v_client_todo`), exposing `meeting_history_date`. Renamed from `f_meeting_history_received`; `accounts.meeting_history_received` no longer feeds it. | `/onboarding` |
