...
| `v_client_onboarding` | Active clients whose first feedback report has not been sent. | `accounts` (active), `tasks` | **Exit gate:** `NOT EXISTS` a `tasks` row for the client (`bcs_account_id`) with `bcs_task_subtype_label = 'Feedback Report Sent'` and `state_label = 'Completed'` — the first completed report drops the client off. Scoped to `onboarding_start_date ≥ 2026-01-01`. The 9 step flags + `filled_count` are **progress display only**, not the gate (they replaced a `filled_count < 9` gate that had gone inert). | `/onboarding` |
