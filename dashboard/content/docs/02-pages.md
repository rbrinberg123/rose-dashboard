...
| `/feedback-manager` | Feedback Reports | `v_feedback_pipeline` | The report pipeline — Open (being written) + Pending Review tables with the pipeline-flow KPIs and Claimed By / Account Manager filters. One row per **Feedback task**, not per event: an event can carry several Feedback / Feedback Report Sent **pairs**, matched by `created_on` rank, each flowing Open → Pending Review → done independently and able to appear in different buckets at once (see [Feedback report pairs](07-business-rules.md#feedback-report-pairs-why-an-event-is-not-a-unique-key)). **All-access** (route-gated only, no row scoping). Own "Feedback Reports" banner. |
